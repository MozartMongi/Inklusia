/**
 * Data contoh untuk pengembangan lokal saja.
 * Akun perusahaan contoh sengaja tidak punya kata sandi yang bisa dipakai masuk.
 * Untuk akun admin production, gunakan `npm run seed:root-admin`.
 */
...
/** Hash acak: akun contoh ada sebagai relasi data, bukan untuk login. */
  ...
  try {
    ...
    for (const job of SEED_JOBS) {
      await client.query(
        ...
        `,
        [job.userId, job.email, unusablePasswordHash()],
      );
      ...
      await client.query(
        ...
        [
          ...
          job.isActive ?? true,
          job.createdAt,
          job.isActive === false ? "menunggu" : "disetujui",
        ],
      );
    }
    ...
    await client.query(
      ...
      [
        DEV_JOB_SEEKER_USER_ID,
        "sari.wulandari@contoh.inklusia.id",
    ...
    console.log(
      "Akun contoh tidak memiliki kata sandi yang bisa dipakai masuk. " +
        "Buat akun admin dengan `npm run seed:root-admin`.",
    );
  } catch (error) {
