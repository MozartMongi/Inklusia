...
import { uploadJobSeekerDocument } from "@/lib/api/job-seeker";
...
export function DocumentsUpload({ profile }: DocumentsUploadProps) {
  ...
  const [ktp, setKtp] = useState<PreviewState>({
    ...
    fileName: profile.ktpPhotoUrl ? "Foto KTP tersimpan" : null,
    objectUrl: null,
    file: null,
  });
  ...
  function applyFile(
    ...
    setter((current) => {
      ...
      return { src: objectUrl, fileName: file.name, objectUrl, file };
    });
  }
  ...
  async function handleSave() {
    ...
    setSaveError("");
    if (!photo.file && !ktp.file) {
      setSaveError("Pilih foto diri atau foto KTP terlebih dahulu.");
      return;
    }
    if (photo.file) {
      const result = await uploadJobSeekerDocument("photo", photo.file);
      if ("error" in result) {
    ...
    if (ktp.file) {
      const result = await uploadJobSeekerDocument("ktp", ktp.file);
      if ("error" in result) {
        setSaveError(result.error);
        return;
      }
