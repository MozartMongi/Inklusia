...
import { addJobSeekerSkill, deleteJobSeekerSkill, updateJobSeekerSkill } from "@/lib/api/job-seeker";
import {
  SKILL_LEVEL_LABEL,
...
export function SkillsManager({ profile }: SkillsManagerProps) {
  ...
  async function handleAdd(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await addJobSeekerSkill({ skillName, level });
    if ("error" in result) {
    ...
    setLevel("menengah");
    setError("");
    setStatus(`${skillName} ditambahkan.`);
  }

  async function handleLevelChange(id: string, nextLevel: SkillLevel) {
    const result = await updateJobSeekerSkill(id, { level: nextLevel });
    if ("error" in result) {
      setError(result.error);
      return;
    }
    setSkills(result.data.skills);
    setStatus("Tingkat kemahiran diperbarui.");
  }

  async function handleRemove(skill: JobSeekerSkill) {
    const result = await deleteJobSeekerSkill(skill.id);
    if ("error" in result) {
      setError(result.error);
      return;
    }
    setSkills(result.data.skills);
    setStatus(`${skill.skillName} dihapus.`);
  }
