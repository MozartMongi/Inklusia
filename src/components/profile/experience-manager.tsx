...
export function ExperienceManager({ profile }: ExperienceManagerProps) {
  ...
  function handleAdd(event: FormEvent<HTMLFormElement>) {
    ...
    setErrors({});
    setStatus(`${experience.position} ditambahkan (simulasi).`);
  }

  function handleRemove(experience: WorkExperience) {
    setExperiences((current) =>
      current.filter((item) => item.id !== experience.id),
    );
    setStatus(`${experience.position} dihapus (simulasi).`);
  }
