...
const CARDS = [
  ...
  },
  {
    ...
    label: "Inquiry karyawan",
    description: (summary: AdminDashboardSummary) =>
      `${summary.openInquiryCount} masih terbuka dari ${summary.inquiryCount} total.`,
...
export function AdminSummaryCards({ summary }: AdminSummaryCardsProps) {
  return (
    <ul
      ...
      className="mb-8 grid list-none grid-cols-1 gap-4 p-0 sm:grid-cols-3"
    >
      {CARDS.map((card) => {
        const description =
