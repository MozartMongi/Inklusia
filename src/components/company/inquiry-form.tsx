...
export function InquiryForm({ companyName, inquiry }: InquiryFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (inquiry) {
      const result = await updateCompanyInquiry(inquiry.id, input);
      if (!result || "error" in result) {
        setErrors({ title: result && "error" in result ? result.error : "Inquiry gagal diperbarui." });
        return;
      }
    } else {
      const result = await createCompanyInquiry(input);
      if ("error" in result) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {isEdit
            ? `Perbarui kebutuhan ${companyName}. Setelah disimpan, admin meninjau ulang sebelum lowongan tampil.`
            : `Inquiry ini untuk ${companyName}. Setelah dikirim, admin meninjau kebutuhan sebelum lowongan tampil di portal.`}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <p role="status" aria-live="polite" className="text-foreground text-sm">
              {status === "saved"
                ? isEdit
                  ? "Kebutuhan diperbarui dan menunggu tinjauan admin."
                  : "Kebutuhan terkirim dan menunggu persetujuan admin."
                : ""}
            </p>
          </div>
        </form>
